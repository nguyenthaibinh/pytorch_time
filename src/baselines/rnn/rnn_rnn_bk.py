...
class RNNEncoder(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, num_layers, cell='lstm', bidirectional=False, batch_first=True):
        """
    ...
    def forward(self, x_input):
        ...
        """
        x_input = x_input.view(x_input.shape[0], x_input.shape[1], self.in_dim)
...
class RNNSeq2Seq(BaseModel):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, output_len, num_layers, cell, bidirectional):
        """
        ...
        self.decoder = RNNDecoder(input_dim=input_dim, hidden_dim=hidden_dim, num_layers=num_layers, cell=cell,
                                  bidirectional=bidirectional, batch_first=True)

    def forward(self, src_seq, trg_len):
        b_size = src_seq.size(0)
        data_dim = src_seq.size(2)

        enc_output, enc_hidden = self.encoder(src_seq)
        dec_input = src_seq[:, -1, :]  # shape: (batch_size, 1, input_size)
        dec_input = dec_input.unsqueeze(1)
        dec_hidden = enc_hidden

        outputs = th.zeros(b_size, trg_len, data_dim)

        for t in range(trg_len):
            try:
                dec_output, dec_hidden = self.decoder(dec_input, dec_hidden)
                outputs[:, t, :] = dec_output[:, 0, :]
...
def get_model(args):
    model = RNNSeq2Seq(input_dim=args.in_dim * args.num_nodes, hidden_dim=args.hidden_dim, output_dim=args.num_nodes,
                       output_len=args.out_len, num_layers=args.layers, device=args.device)
