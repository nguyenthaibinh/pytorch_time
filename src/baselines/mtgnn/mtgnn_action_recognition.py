...
class MTGNNCLS(BaseModel):
    def __init__(self, gcn_true, buildA_true, gcn_depth, num_nodes, device, predefined_A=None, static_feat=None, dropout=0.3, subgraph_size=20, node_dim=40, dilation_exponential=1, conv_channels=32, residual_channels=32, skip_channels=64, end_channels=128, seq_length=12, in_dim=2, out_dim=12, layers=3, propalpha=0.05, tanhalpha=3, layer_norm_affline=True,
        ...
        # fcn for prediction
        self.fcn = nn.Conv2d(skip_channels, n_classes, kernel_size=1)

    def forward(self, input, idx=None):
        # input: B, C, T, N
        B = input.size(0)
        input = input.permute(0, 1, 3, 2)   # B, C, N, T
        seq_len = input.size(3)
        assert seq_len == self.seq_length, f'input sequence length ({seq_len}) not equal to ' \
                                           f'preset sequence length ({self.seq_length})'
...
def get_model(args, adj_matrix):
    ...
    model = MTGNNCLS(gcn_true=args.gcn_true, buildA_true=args.buildA_true, gcn_depth=args.gcn_depth,
                     num_nodes=args.num_nodes, device=args.device, predefined_A=A, in_dim=args.in_dim,
                     seq_length=args.seq_len, n_classes=args.n_classes)
