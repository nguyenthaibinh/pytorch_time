...
from evolve_gnn.gnn import TGC

class MTGNN_GRAPH(BaseModel):
    def __init__(self, gcn_true, buildA_true, gcn_depth, num_nodes, kernel_set, device, predefined_A=None,
        ...
        super(MTGNN_GRAPH, self).__init__()
        ...
        for i in range(1):
            ...
            for j in range(1,layers+1):
                if dilation_exponential > 1:
                    ...
                    rf_size_j_prev = int(rf_size_i + (kernel_size - 1) *
                                         (dilation_exponential ** (j - 1) - 1) / (dilation_exponential - 1))
                else:
                    rf_size_j = rf_size_i+j*(kernel_size-1)
                    rf_size_j_prev = rf_size_i + (j - 1) * (kernel_size - 1)
                ...
                if self.gcn_true:
                    ...
                    self.gconv1.append(TGC(conv_channels, residual_channels))
                    self.gconv2.append(TGC(conv_channels, residual_channels))

                if self.seq_length > self.receptive_field:
                    self.g_norm.append(LayerNorm((residual_channels, num_nodes, self.seq_length - rf_size_i + 1),
                                                 elementwise_affine=layer_norm_affline))
                else:
                    self.g_norm.append(LayerNorm((residual_channels, num_nodes,
                                                  self.receptive_field - rf_size_j_prev + 1),
                                                 elementwise_affine=layer_norm_affline))
    ...
    def forward(self, input, idx=None):
        ...
        for i in range(self.layers):
            ...
            if self.gcn_true:
                ...
                if idx is None:
                    x = self.g_norm[i](x, self.idx)
                else:
                    x = self.g_norm[i](x, idx)
...
def get_model(args, adj_matrix):
    ...
    model = MTGNN_GRAPH(gcn_true=args.gcn_true, buildA_true=args.buildA_true, gcn_depth=args.gcn_depth,
