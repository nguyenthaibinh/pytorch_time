...
from baselines.graph_wavenet.graph import GraphConstructor, MixPropGNN
...
class GWNET_ADP(BaseModel):
    def __init__(self, device, num_nodes, dropout=0.3, supports=None, predefined_A=None, gcn_bool=True, addaptadj=True,
        ...
        self.gc = GraphConstructor(nnodes=num_nodes, k=20, dim=32, device=device, alpha=3, static_feat=None)
        ...
        for b in range(blocks):
            ...
            for i in range(layers):
                ...
                if self.gcn_bool:
                    ...
                    self.gconv1.append(MixPropGNN(dilation_channels, residual_channels, gcn_depth=2,
                                                  dropout=dropout, alpha=0.05))
                    self.gconv2.append(MixPropGNN(dilation_channels, residual_channels, gcn_depth=2,
                                                  dropout=dropout, alpha=0.05))
...
def get_model(args, adj_matrix):
    ...
    model = GWNET_ADP(in_dim=args.in_dim, num_nodes=args.num_nodes, device=args.device, gcn_bool=args.gcn_true,
                      supports=None, predefined_A=A)
