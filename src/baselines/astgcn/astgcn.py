...
def get_model(args, adj_mx):
    L_tilde = scaled_Laplacian(adj_mx)
    cheb_polynomials = [torch.from_numpy(i).type(torch.FloatTensor).to(args.device)
                        for i in cheb_polynomial(L_tilde, args.K)]
    model = ASTGCN_submodule(args.device, args.nb_block, args.in_channels, args.K, args.nb_chev_filter,
                             args.nb_time_filter, args.time_strides, cheb_polynomials, args.num_for_predict,
                             args.len_input, args.num_nodes)
