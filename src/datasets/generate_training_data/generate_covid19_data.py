...
def generate_train_val_test(args):
    ...
    fin = open(data_file)
    data = np.loadtxt(fin, delimiter=',', skiprows=1)       # (52560, 137)
    data = np.expand_dims(data, axis=2)

    data, scaler = normalize_dataset(data, normalizer=args.normalizer, column_wise=args.column_wise)
    ...
    print(f'data_mean: {data.mean():.4f}', file=f)
    print(f'data_std: {data.std():.4f}', file=f)
...
if __name__ == "__main__":
    ...
    dataset = 'covid19_death_global'
    data_file = f'/home/nbinh/datasets/time_series/{dataset}/{dataset}.csv'
    ...
    if args.normalizer == 'None':
        dataset_name = f'{Path(data_file).stem}_original_{args.window}x{args.horizon}'
    else:
